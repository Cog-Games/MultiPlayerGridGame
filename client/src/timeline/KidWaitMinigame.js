export class KidWaitMinigame {
  ...
  drawJumper(ctx) {
    const j = this.jumper;
    const squash = j.onGround ? j.bounce * 4 : 0;

    ctx.save();
    ctx.translate(j.x + j.width / 2, j.y + j.height / 2 + squash / 2);
    ctx.scale(1 + squash / 80, 1 - squash / 80);
    ctx.translate(-j.width / 2, -j.height / 2);
  ...
  drawSparkles(ctx) {
    ...
    this.sparkles.forEach(sparkle => {
      const opacity = Math.max(0, Math.min(1, sparkle.life / 520));
      ctx.save();
