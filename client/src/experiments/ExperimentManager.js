      ...
      try {
        ...
        if (td) {
          ...
          if (ft === 'llm' || ft === 'llm-tom') {
            ...
            td.partnerAgentType = CONFIG?.kids?.committedAgentLabel || 'SA-model';
  ...
  async logCurrentAIModel() {
    try {
      ...
      if (normalizedType === 'llm' || normalizedType === 'llm-tom') {
        ...
        console.log(`🤖 AI partner: AlwaysSignalAgent (${CONFIG?.kids?.committedAgentLabel || 'SA-model'}, lambda=${cfg.lambda}, rho=${cfg.alpha}, score=${cfg.score || 'costly_mixture'}, policy=${policyKind})`);
