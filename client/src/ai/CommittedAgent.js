...
// Bayesian joint-goal selection with one fitted reliance parameter on inferred intent.
// - Maintain P_intent(joint goal) inferred from observed actions using RL policy likelihood.
// - After joint goal detected and a new goal appears, re-sample a joint goal each step using:
//     W_lambda(g) ∝ exp(beta * EU(g)) * P_intent(joint goal)^lambda
// - Movement is generated by joint-RL restricted to the sampled joint goal for that step.
...
export class CommittedAgent {
  ...
  getAIAction(gameState, trialData, aiPlayerNumber = 2) {
    ...
    // After joint detection + new-goal: re-sample a joint goal every step.
    ...
    // Act using the AI component of a joint-RL policy that only considers the sampled joint goal.
