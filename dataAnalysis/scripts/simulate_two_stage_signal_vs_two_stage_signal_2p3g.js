...
import { TwoStageSignalAgent } from '../../client/src/ai/TwoStageSignalAgent.js';
...
const DEFAULT_OUTPUT_DIR = 'dataAnalysis/model_model/two_stage_signal_agent/outputs/simulations';
const DEFAULT_RAW_OUTPUT_DIR = 'dataAnalysis/raw_data/model_model_simulations/two_stage_signal_agent';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === '--tau' && argv[i + 1]) out.tau = Number(argv[++i]);
    else if (arg === '--alpha' && argv[i + 1]) out.alpha = Number(argv[++i]);
    else if (arg === '--eta' && argv[i + 1]) out.eta = Number(argv[++i]);
    else if (arg === '--beta' && argv[i + 1]) out.beta = Number(argv[++i]);
    else if ((arg === '--gate-sharpness' || arg === '--k') && argv[i + 1]) out.gateSharpness = Number(argv[++i]);
    else if ((arg === '--signal-mode' || arg === '--score') && argv[i + 1]) out.signalMode = String(argv[++i]);
...
function buildRawTrialRecord(trialData, mapId, sessionIndex, design) {
  ...
  return {
    ...
    player1FinalReachedGoal: trialData.player1FinalReachedGoal,
    player2FinalReachedGoal: trialData.player2FinalReachedGoal,
    twoStageSignalAgentPlayer1GoalPosterior: trialData.twoStageSignalAgentPlayer1GoalPosterior,
    twoStageSignalAgentPlayer2GoalPosterior: trialData.twoStageSignalAgentPlayer2GoalPosterior,
    twoStageSignalAgentPlayer1Gate: trialData.twoStageSignalAgentPlayer1Gate,
    twoStageSignalAgentPlayer2Gate: trialData.twoStageSignalAgentPlayer2Gate,
    twoStageSignalAgentSignalMode: trialData.twoStageSignalAgentSignalMode,
    twoStageSignalAgentPlayer1SignalMode: trialData.twoStageSignalAgentPlayer1SignalMode,
    twoStageSignalAgentPlayer2SignalMode: trialData.twoStageSignalAgentPlayer2SignalMode,
    twoStageSignalAgentPlayer1GoalWeights: trialData.twoStageSignalAgentPlayer1GoalWeights,
    twoStageSignalAgentPlayer2GoalWeights: trialData.twoStageSignalAgentPlayer2GoalWeights,
    twoStageSignalAgentPlayer1ActionProbabilities: trialData.twoStageSignalAgentPlayer1ActionProbabilities,
    twoStageSignalAgentPlayer2ActionProbabilities: trialData.twoStageSignalAgentPlayer2ActionProbabilities,
    twoStageSignalAgentPlayer1History: trialData.twoStageSignalAgentPlayer1History,
    twoStageSignalAgentPlayer2History: trialData.twoStageSignalAgentPlayer2History,
...
function runSimulation({ sessions, trialsPerSession, seed, sessionOffset, lambda, tau, alpha, eta, beta, gateSharpness, signalMode }) {
  ...
  const originalConfig = { ...(CONFIG.game.agent.twoStageSignal || {}) };

  CONFIG.game.players.player1.type = 'twoStageSignalAgent';
  CONFIG.game.players.player2.type = 'twoStageSignalAgent';
  CONFIG.game.agent.type = 'joint';
  CONFIG.game.experiments.order = ['2P3G'];
  CONFIG.game.experiments.numTrials['2P3G'] = trialsPerSession;
  CONFIG.game.agent.twoStageSignal = { lambda, tau, alpha, eta, beta, gateSharpness, signalMode };
  ...
  try {
    ...
    return withSuppressedConsole(() => withSynchronousTimers(() => {
      ...
      for (let localSessionIndex = 0; localSessionIndex < sessions; localSessionIndex++) {
        ...
        withSeededRandom(sessionSeed, () => {
          ...
          const agent1 = new TwoStageSignalAgent({ rlAgent: sharedRl, lambda, tau, alpha, eta, beta, gateSharpness, signalMode });
          const agent2 = new TwoStageSignalAgent({ rlAgent: sharedRl, lambda, tau, alpha, eta, beta, gateSharpness, signalMode });
    ...
    CONFIG.game.agent.twoStageSignal = originalConfig;
  }
}
...
function main() {
  ...
  const rawOutputDir = path.resolve(args.rawOutputDir);
  fs.mkdirSync(outputDir, { recursive: true });
  fs.mkdirSync(rawOutputDir, { recursive: true });

  const suffix = [
    `signal_${args.signalMode}`,
    `beta_${formatNumberForPath(args.beta)}`,
    `lambda_${formatNumberForPath(args.lambda)}`,
    `tau_${formatNumberForPath(args.tau)}`,
    `alpha_${formatNumberForPath(args.alpha)}`,
    `eta_${formatNumberForPath(args.eta)}`,
    `sessions_${args.sessionOffset}_to_${args.sessionOffset + args.sessions - 1}`
  ].join('_');
  const summaryPath = path.join(outputDir, `two_stage_signal_vs_two_stage_signal_2p3g_summary_${suffix}.json`);
  const trialsPath = path.join(outputDir, `two_stage_signal_vs_two_stage_signal_2p3g_trials_${suffix}.json`);
  const rawTrialsPath = path.join(rawOutputDir, `two_stage_signal_vs_two_stage_signal_2p3g_raw_trials_${suffix}.json`);
