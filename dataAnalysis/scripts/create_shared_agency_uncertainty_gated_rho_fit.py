#!/usr/bin/env python3
"""Fit a one-parameter uncertainty-gated RSA signal policy."""
...
OUT_DIR = MODEL_ROOT / "shared_agency_uncertainty_gated_rho_fit"
ASSET_DIR = OUT_DIR / "assets"
NOTEBOOK_DIR = MODEL_ROOT / "joint_rl" / "notebooks" / "shared_agency_uncertainty_gated_rho_fit"
NOTEBOOK_PATH = NOTEBOOK_DIR / "shared_agency_uncertainty_gated_rho_fit.ipynb"
HTML_PATH = MODEL_ROOT / "shared_agency_uncertainty_gated_rho_fit.html"
...
def rsa_goal_action_prob(obs: StepObs, goal_idx: int, alpha: float = FIXED_ALPHA) -> float:
    ...
    if not math.isfinite(total) or total <= 0:
        return max(EPS, float(base[obs.observed_idx] / max(EPS, float(np.sum(base)))))
    return max(EPS, float(unnormalized[obs.observed_idx] / total))


def uncertainty_gated_likelihood(obs: StepObs, rho_max: float, lambda_value: float = FIXED_LAMBDA, alpha: float = FIXED_ALPHA) -> float:
    weights = goal_weights(obs, lambda_value)
    uncertainty = normalized_entropy(obs.posterior)
    rho_t = min(1.0, max(0.0, float(rho_max) * uncertainty))
    per_goal = np.zeros(len(weights), dtype=np.float64)
    for goal_idx in range(len(weights)):
        base_prob = max(EPS, float(obs.action_probs_by_goal[goal_idx, obs.observed_idx]))
        rsa_prob = rsa_goal_action_prob(obs, goal_idx, alpha)
        per_goal[goal_idx] = (1.0 - rho_t) * base_prob + rho_t * rsa_prob
...
def gated_nll(observations: Sequence[StepObs], rho_max: float) -> float:
    return nll(observations, lambda obs: uncertainty_gated_likelihood(obs, rho_max))


def no_signal_nll(observations: Sequence[StepObs]) -> float:
    return nll(
        observations,
        lambda obs: step_report.rsa_signal_likelihood(obs, FIXED_LAMBDA, 0.0),
    )
...
def constant_rsa_nll(observations: Sequence[StepObs]) -> float:
    return nll(
        observations,
        lambda obs: step_report.rsa_signal_likelihood(obs, FIXED_LAMBDA, FIXED_ALPHA),
    )
...
def trial_best_nll(observations: Sequence[StepObs]) -> float:
    return nll(
        observations,
        lambda obs: step_report.rsa_signal_likelihood(obs, 0.15, 0.5),
    )
...
def fit_rho(observations: Sequence[StepObs]) -> tuple[pd.DataFrame, Dict[str, Any]]:
    rows = [
        {
            ...
            "lambda": FIXED_LAMBDA,
            "alpha": FIXED_ALPHA,
            "negative_log_likelihood": gated_nll(observations, float(rho)),
            "fit_stage": "coarse",
        }
    ...
    refine_rhos = step_report.refinement_values(float(best_coarse["rho_max"]), COARSE_RHOS, 0.0, 1.0)
    seen = {round(float(row["rho_max"]), 10) for row in rows}
    ...
    df = pd.DataFrame(rows).sort_values("rho_max").reset_index(drop=True)
...
def cv_for_window(observations: Sequence[StepObs]) -> pd.DataFrame:
    ...
    for fold in range(CV_FOLDS):
        ...
        candidates = [
            ...
            ("uncertainty-gated RSA", float(best["rho_max"]), gated_nll(train, float(best["rho_max"])), gated_nll(test, float(best["rho_max"])), 1),
            ("trial-level best lambda=.15 alpha=.5", np.nan, trial_best_nll(train), trial_best_nll(test), 0),
        ]
        for model, rho_max, train_nll, test_nll, n_params in candidates:
            rows.append(
                {
                    "fold": fold,
                    "model": model,
                    "rho_max": rho_max,
                    "lambda": FIXED_LAMBDA if model != "trial-level best lambda=.15 alpha=.5" else 0.15,
                    "alpha": FIXED_ALPHA if model != "trial-level best lambda=.15 alpha=.5" else 0.5,
                    "n_params": n_params,
...
def summarize_models(observations: Sequence[StepObs], grid: pd.DataFrame, best: Dict[str, Any], cv_df: pd.DataFrame) -> pd.DataFrame:
    candidates = [
        ("no signaling, lambda=.2", 0.0, no_signal_nll(observations), 0),
        ("constant RSA alpha=1", 1.0, constant_rsa_nll(observations), 0),
        ("uncertainty-gated RSA", float(best["rho_max"]), float(best["negative_log_likelihood"]), 1),
        ("trial-level best lambda=.15 alpha=.5", np.nan, trial_best_nll(observations), 0),
    ]
    rows = []
    for model, rho_max, insample, n_params in candidates:
...
def plot_rho_sweep(grid: pd.DataFrame, best: Dict[str, Any], path: Path, title: str) -> None:
    df = grid.sort_values("rho_max")
    fig, ax = plt.subplots(figsize=(8.5, 5.2))
    ax.plot(df["rho_max"], df["negative_log_likelihood"], marker="o", linewidth=2.2, color="#4f79a8")
    ax.axvline(float(best["rho_max"]), color="#d62728", linestyle="--", linewidth=1.4, label=f"best rho={float(best['rho_max']):g}")
...
def plot_model_summary(summary: pd.DataFrame, path: Path, title: str) -> None:
    ...
    colors = ["#59a14f" if model == "uncertainty-gated RSA" else "#4f79a8" for model in df["model"]]
...
def write_notebook() -> None:
    ...
    nb = {
        "cells": [
            {"cell_type": "markdown", "metadata": {}, "source": ["# Uncertainty-Gated Shared-Agency RSA Fit\n"]},
            {
                "cell_type": "code",
...
def write_html(results: Dict[str, Dict[str, Any]]) -> None:
  ...
  <section class="panel">
    ...
    <p class="note">Actions={info['actions']}; mean normalized posterior entropy={info['mean_uncertainty']:.3f}. Fixed lambda={FIXED_LAMBDA:g}, fixed alpha={FIXED_ALPHA:g}; only rho_max is fitted.</p>
...
<title>Uncertainty-Gated Shared-Agency RSA Fit</title>
<style>
...
<body>
<header>
  <h1>Uncertainty-Gated Shared-Agency RSA Fit</h1>
  <p class="note">Version A: fixed lambda=0.2, fixed RSA alpha=1, and fit one parameter rho_max in rho_t = rho_max U_t, where U_t is normalized entropy of the joint-goal posterior.</p>
  <div class="links">
...
<main>
  <section class="panel">
    <h2>Model</h2>
    <p class="note">For each candidate goal, action likelihood is a mixture of base unshaped JointRL and RSA/log-posterior signaling. The mixture weight is high only when posterior uncertainty is high.</p>
    <p><code>pi(a|g,s,P) = (1-rho_max U(P)) pi_base(a|s,g) + rho_max U(P) pi_RSA(a|s,g,P; alpha=1)</code></p>
  </section>
  {''.join(html_sections)}
</main>
</body>
...
def analyze_window(key: str, observations: Sequence[StepObs]) -> Dict[str, Any]:
    ...
    summary_df = summarize_models(observations, grid, best, cv_df)
    ...
    plot_rho_sweep(grid, best, rho_plot, f"{WINDOWS[key]['label']}: rho_max sweep")
...
def main() -> None:
    ...
    summary = {
        ...
        "windows": {
            key: {
                ...
                "best": {k: v for k, v in value["best"].items() if k in {"rho_max", "negative_log_likelihood", "fit_stage"}},
    ...
    (OUT_DIR / "uncertainty_gated_rho_fit_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
