#!/usr/bin/env python3
"""Validate a one-parameter value-safe tie-break signaling policy."""
...
OUT_DIR = MODEL_ROOT / "shared_agency_tiebreak_signal_fit"
ASSET_DIR = OUT_DIR / "assets"
NOTEBOOK_DIR = MODEL_ROOT / "joint_rl" / "notebooks" / "shared_agency_tiebreak_signal_fit"
NOTEBOOK_PATH = NOTEBOOK_DIR / "shared_agency_tiebreak_signal_fit.ipynb"
HTML_PATH = MODEL_ROOT / "shared_agency_tiebreak_signal_fit.html"
...
def logsumexp(values: np.ndarray) -> float:
    ...
    return float(max_value + math.log(max(EPS, float(np.sum(np.exp(finite - max_value))))))


def own_soft_q_values(obs: StepObs, goal_idx: int) -> np.ndarray:
    q = unshaped_q_values(obs.self_pos, obs.other_pos, [obs.goals[goal_idx]]).reshape(len(ACTIONS), len(ACTIONS))
    return np.asarray([logsumexp(SOFTMAX_BETA * q[action_idx, :]) / SOFTMAX_BETA for action_idx in range(len(ACTIONS))], dtype=np.float64)


def safe_action_mask(obs: StepObs, goal_idx: int) -> np.ndarray:
    own_q = own_soft_q_values(obs, goal_idx)
    best = float(np.max(own_q[np.isfinite(own_q)])) if np.any(np.isfinite(own_q)) else 0.0
    return own_q >= best - VALUE_EPSILON - VALUE_TOL
...
def value_safe_legible_policy(obs: StepObs, goal_idx: int) -> np.ndarray:
    mask = safe_action_mask(obs, goal_idx)
    if not np.any(mask):
        return np.maximum(EPS, obs.action_probs_by_goal[goal_idx, :] / max(EPS, float(np.sum(obs.action_probs_by_goal[goal_idx, :]))))
    scores = np.full(len(ACTIONS), -np.inf, dtype=np.float64)
    for action_idx in range(len(ACTIONS)):
        if mask[action_idx]:
            scores[action_idx] = LEGIBILITY_BETA * posterior_lift_score(obs, goal_idx, action_idx)
    max_score = float(np.max(scores[np.isfinite(scores)]))
    probs = np.where(mask, np.exp(np.clip(scores - max_score, -700, 700)), 0.0)
    total = float(np.sum(probs))
    if not math.isfinite(total) or total <= 0:
        probs = mask.astype(np.float64)
        total = float(np.sum(probs))
    return np.maximum(EPS, probs / max(EPS, total))


def tiebreak_goal_policy(obs: StepObs, goal_idx: int, rho: float) -> np.ndarray:
    base = np.maximum(EPS, obs.action_probs_by_goal[goal_idx, :])
    base = base / max(EPS, float(np.sum(base)))
    legible = value_safe_legible_policy(obs, goal_idx)
    probs = (1.0 - float(rho)) * base + float(rho) * legible
    probs = np.maximum(EPS, probs)
    return probs / max(EPS, float(np.sum(probs)))


def tiebreak_likelihood(obs: StepObs, rho: float, lambda_value: float = FIXED_LAMBDA) -> float:
    weights = goal_weights(obs, lambda_value)
    per_goal = np.zeros(len(weights), dtype=np.float64)
    for goal_idx in range(len(weights)):
        per_goal[goal_idx] = tiebreak_goal_policy(obs, goal_idx, rho)[obs.observed_idx]
...
def tiebreak_nll(observations: Sequence[StepObs], rho: float) -> float:
    return nll(observations, lambda obs: tiebreak_likelihood(obs, rho))
...
def expected_policy_metrics(observations: Sequence[StepObs], rho: float) -> Dict[str, float]:
    if not observations:
        return {
            ...
            "expected_value_safe_mass": np.nan,
        }
    signal_values = []
    btom_values = []
    safe_values = []
    for obs in observations:
        ...
        expected_safe = 0.0
        for goal_idx in range(len(weights)):
            probs = tiebreak_goal_policy(obs, goal_idx, rho)
            expected_signal += float(weights[goal_idx]) * float(np.sum(probs * signal_matrix[goal_idx, :]))
            expected_safe += float(weights[goal_idx]) * float(np.sum(probs * safe_action_mask(obs, goal_idx).astype(np.float64)))
            for action_idx in range(len(ACTIONS)):
                post = revealed_posterior(obs.posterior, obs.action_probs_by_goal, action_idx)
    ...
    rows = [
        {
            "rho": float(rho),
            "lambda": FIXED_LAMBDA,
            "negative_log_likelihood": tiebreak_nll(observations, float(rho)),
...
def cv_for_window(observations: Sequence[StepObs]) -> pd.DataFrame:
    ...
    for fold in range(CV_FOLDS):
        ...
        candidates = [
            ...
            ("value-safe tie-break signaling", rho, tiebreak_nll(train, rho), tiebreak_nll(test, rho), 1),
        ]
...
def summarize_models(observations: Sequence[StepObs], best: Dict[str, Any], cv_df: pd.DataFrame) -> pd.DataFrame:
    ...
    candidates = [
        ...
        ("value-safe tie-break signaling", rho, float(best["negative_log_likelihood"]), 1),
    ]
    rows = []
...
def plot_rho_sweep(grid: pd.DataFrame, best: Dict[str, Any], path: Path, title: str) -> None:
    ...
    series = [
        ...
        ("expected_value_safe_mass", "Expected Value-Safe Action Mass", "#b07aa1"),
    ]
...
def plot_model_summary(summary: pd.DataFrame, path: Path, title: str) -> None:
    ...
    colors = ["#4f79a8", "#59a14f"][: df.shape[0]]
    panels = [
...
def write_notebook() -> None:
    ...
    nb = {
        "cells": [
            {"cell_type": "markdown", "metadata": {}, "source": ["# Shared-Agency Value-Safe Tie-Break Signaling Fit\n"]},
            {
                "cell_type": "code",
...
def write_html(results: Dict[str, Dict[str, Any]]) -> None:
  ...
  <section class="panel">
    ...
    <p class="note">Actions={info['actions']}. Fixed lambda={FIXED_LAMBDA:g}; value epsilon={VALUE_EPSILON:g}; legibility beta={LEGIBILITY_BETA:g}. Only rho is fitted. Goal weights are unchanged for every rho.</p>
...
<title>Shared-Agency Value-Safe Tie-Break Signaling Fit</title>
<style>
...
<body>
<header>
  <h1>Shared-Agency Value-Safe Tie-Break Signaling Fit</h1>
  <p class="note">Fixed lambda=0.2. This validates a one-parameter signaling policy that changes only the action policy, not the goal-selection weights.</p>
  <div class="links">
...
<main>
  <section class="panel">
    <h2>Model</h2>
    <p class="note">For each sampled goal, signaling can only reweight actions in the value-safe set. The free parameter rho mixes the unshaped JointRL base policy with a legibility-maximizing tie-break policy.</p>
    <p><code>pi(a|g) = (1-rho) pi_base(a|g) + rho pi_tiebreak(a|g)</code></p>
    <p><code>A_safe(g) = actions whose soft Q is within epsilon=0 of the best action</code></p>
  </section>
  {''.join(html_sections)}
</main>
</body>
...
def analyze_window(key: str, observations: Sequence[StepObs]) -> Dict[str, Any]:
    ...
    plot_rho_sweep(grid, best, rho_plot, f"{WINDOWS[key]['label']}: rho sweep")
...
def main() -> None:
    ...
    summary = {
        ...
        "legibility_beta": LEGIBILITY_BETA,
        "windows": {
            key: {
    ...
    (OUT_DIR / "tiebreak_signal_fit_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
