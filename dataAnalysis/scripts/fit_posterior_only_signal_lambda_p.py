#!/usr/bin/env python3
"""Fit PosteriorOnlySignalAgent lambda x mixture-p against trial/player-level human data."""
...
SIM_SCRIPT = PROJECT_ROOT / "dataAnalysis" / "scripts" / "simulate_posterior_only_signal_vs_posterior_only_signal_2p3g.js"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "dataAnalysis" / "model_model" / "signal_agent" / "outputs" / "signal_agent_posterior_only_lambda_p_fit"
DEFAULT_RAW_DIR = PROJECT_ROOT / "dataAnalysis" / "raw_data" / "model_model_simulations" / "signal_agent" / "posterior_only_lambda_p_fit"

COARSE_LAMBDAS = [0.0, 0.05, 0.1, 0.15, 0.3, 0.5, 1.0]
COARSE_PS = [0.0, 0.1, 0.25, 0.375, 0.5, 0.75, 1.0]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--score", choices=["mixture"], default="mixture")
...
def raw_path_for(raw_dir: Path, beta: float, lambda_value: float, p_signal: float, sessions: int, offset: int = 0) -> Path:
    suffix = (
        f"beta_{fmt(beta)}_lambda_{fmt(lambda_value)}_alpha_{fmt(p_signal)}_"
...
def run_simulation(args: argparse.Namespace, lambda_value: float, p_signal: float) -> Path:
    raw_path = raw_path_for(args.raw_output_dir, args.beta, lambda_value, p_signal, args.sessions)
    ...
    if result.returncode != 0:
        ...
        if result.stderr:
            print(result.stderr)
        raise RuntimeError(f"Simulation failed for lambda={lambda_value:g}, p={p_signal:g}")
...
def measure_row(lambda_value: float, p_signal: float, raw_trials: List[Dict[str, Any]], sim_df: pd.DataFrame) -> Dict[str, float]:
    row: Dict[str, float] = {"lambda": lambda_value, "p_signal": p_signal, "alpha": p_signal}
    for prefix, condition in [("average", None), ("equal", "equal_to_both")]:
        rows = comparison_rows("samplePosteriorOnlyGoalAndSignal_fromStart", raw_trials, sim_df, condition)
...
def setting_key(lambda_value: float, p_signal: float) -> Tuple[float, float]:
    return round(lambda_value, 10), round(p_signal, 10)


def plot_lambda_p_heatmaps(grid_df: pd.DataFrame, best_row: Dict[str, Any], path: Path, scope: str, title: str) -> None:
    panels = [
    ...
    p_values = sorted(grid_df["p_signal"].dropna().unique())
    ...
    best_y = p_values.index(float(best_row["p_signal"])) if float(best_row["p_signal"]) in p_values else None

    for ax, (panel_title, col) in zip(axes.ravel(), panels):
        pivot = grid_df.pivot_table(index="p_signal", columns="lambda", values=col, aggfunc="mean")
        pivot = pivot.reindex(index=p_values, columns=lambda_values)
        ...
        ax.set_yticklabels([f"{v:g}" for v in p_values])
...
def write_notebook(output_dir: Path, best_row: Dict[str, Any]) -> Path:
    nb_dir = PROJECT_ROOT / "dataAnalysis" / "model_model" / "signal_agent" / "notebooks" / "signal_agent_posterior_only_lambda_p_fit"
    nb_dir.mkdir(parents=True, exist_ok=True)
    nb_path = nb_dir / "PosteriorOnlySignalAgent_from_start_lambda_p_results.ipynb"
    cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": [
                "# samplePosteriorOnlyGoalAndSignal_fromStart Lambda x p Fit\n",
                "\n",
                "Always-on posterior timing with posterior-only goal selection and SignalAgent mixture action policy.\n",
                f"Best setting: lambda = {float(best_row['lambda']):g}, p = {float(best_row['p_signal']):g}.\n",
            ],
        },
        {
            "cell_type": "code",
            ...
            "source": [
                ...
                "grid = pd.read_csv(OUT / 'posterior_only_signal_lambda_p_grid.csv')\n",
                "best_metrics = pd.read_csv(OUT / 'posterior_only_signal_lambda_p_best_metrics.csv')\n",
                "all_summary = pd.read_csv(OUT / 'all_distance_posterior_only_signal_human_4panel_summary.csv')\n",
                "equal_summary = pd.read_csv(OUT / 'equal_to_both_posterior_only_signal_human_4panel_summary.csv')\n",
            ],
        },
...
def evaluate_setting(
    ...
    raw_path = run_simulation(args, lambda_value, p_signal)
    raw_trials = load_raw(raw_path)
    sim_df = add_measures(long_player_rows(raw_trials, "samplePosteriorOnlyGoalAndSignal_fromStart"))
    ...
    row.update(measure_row(lambda_value, p_signal, raw_trials, sim_df))
...
def main() -> None:
    ...
    ps = parse_float_grid(args.ps, COARSE_PS)
    ...
    coarse_settings = [(float(l), float(p)) for l, p in product(lambdas, ps)]
    if args.max_grid is not None:
        coarse_settings = coarse_settings[: args.max_grid]

    for lambda_value, p_signal in coarse_settings:
        key = setting_key(lambda_value, p_signal)
        if key in seen:
            continue
        seen.add(key)
        row, _ = evaluate_setting(args, lambda_value, p_signal, target)
    ...
    if not args.skip_refine and args.max_grid is None:
        ...
        refine_ps = refinement_values(float(best_coarse["p_signal"]), ps, 0.0, 1.0)
        for lambda_value, p_signal in product(refine_lambdas, refine_ps):
            key = setting_key(lambda_value, p_signal)
            if key in seen:
                continue
            seen.add(key)
            row, _ = evaluate_setting(args, float(lambda_value), float(p_signal), target)
            row["fit_stage"] = "refine"
            grid_rows.append(row)

    grid_df = pd.DataFrame(grid_rows).sort_values(["lambda", "p_signal"]).reset_index(drop=True)
    ...
    best_df = add_measures(long_player_rows(best_raw, "samplePosteriorOnlyGoalAndSignal_fromStart"))

    model_label = (
        "samplePosteriorOnlyGoalAndSignal_fromStart\n"
        f"(lambda={float(best_row['lambda']):g}, p={float(best_row['p_signal']):g})"
    )
    ...
    grid_path = args.output_dir / "posterior_only_signal_lambda_p_grid.csv"
    best_metrics_path = args.output_dir / "posterior_only_signal_lambda_p_best_metrics.csv"
    equal_csv = args.output_dir / "equal_to_both_posterior_only_signal_human_4panel_summary.csv"
    all_csv = args.output_dir / "all_distance_posterior_only_signal_human_4panel_summary.csv"
    grid_df.to_csv(grid_path, index=False)
    pd.DataFrame([best_row]).to_csv(best_metrics_path, index=False)
    equal_df.to_csv(equal_csv, index=False)
    all_df.to_csv(all_csv, index=False)

    average_heatmap = args.output_dir / "posterior_only_signal_lambda_p_average_4measure_heatmaps.png"
    equal_heatmap = args.output_dir / "posterior_only_signal_lambda_p_equal_to_both_4measure_heatmaps.png"
    plot_lambda_p_heatmaps(
        grid_df,
        best_row,
        average_heatmap,
        "average",
        "samplePosteriorOnlyGoalAndSignal_fromStart: lambda x p sweep, average all 2P3G",
    )
    plot_lambda_p_heatmaps(
        grid_df,
        best_row,
        equal_heatmap,
        "equal",
        "samplePosteriorOnlyGoalAndSignal_fromStart: lambda x p sweep, equal-to-both",
    )
    plot_comparison(
        equal_df,
        args.output_dir / "equal_to_both_posterior_only_signal_human_4panel.png",
        "Equal-to-Both Only: samplePosteriorOnlyGoalAndSignal_fromStart and Human-Human",
    )
    plot_comparison(
        all_df,
        args.output_dir / "all_distance_posterior_only_signal_human_4panel.png",
        "All Distance Conditions: samplePosteriorOnlyGoalAndSignal_fromStart and Human-Human",
    )
    notebook_path = None if args.skip_notebook else write_notebook(args.output_dir, best_row)

    summary = {
        "model": "samplePosteriorOnlyGoalAndSignal_fromStart",
        "implementation": "PosteriorOnlySignalAgent",
        ...
        "max_expected_settings": int(len(lambdas) * len(ps) + 25),
    ...
    summary_path = args.output_dir / "posterior_only_signal_lambda_p_fit_summary.json"
