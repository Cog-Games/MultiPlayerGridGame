...
import { AlwaysCommittedAgent } from '../../client/src/ai/AlwaysCommittedAgent.js';
...
const DEFAULT_OUTPUT_DIR = 'dataAnalysis/analyses/outputs/model_model/always_committed_agent/always_committed_vs_always_committed_simulation';
...
function summarizeTrial(trialData, mapId, sessionIndex) {
  ...
  return {
    ...
    player1SampledJointGoal: trialData.alwaysCommittedAgentPlayer1SampledJointGoal,
    player2SampledJointGoal: trialData.alwaysCommittedAgentPlayer2SampledJointGoal,
...
function buildRawTrialRecord(trialData, mapId, sessionIndex, design) {
  ...
  return {
    sessionIndex,
    participantId_player1: `always_committed_session_${sessionIndex}_player1`,
    participantId_player2: `always_committed_session_${sessionIndex}_player2`,
    partnerType: 'always_committed',
    experimentType: '2P3G',
    trialIndex: trialData.trialIndex,
    mapId,
    distanceCondition: trialData.distanceCondition,
    ...
    player1SampledJointGoal: trialData.alwaysCommittedAgentPlayer1SampledJointGoal,
    player2SampledJointGoal: trialData.alwaysCommittedAgentPlayer2SampledJointGoal,
    alwaysCommittedAgentPlayer1SampledJointGoal: trialData.alwaysCommittedAgentPlayer1SampledJointGoal,
    alwaysCommittedAgentPlayer2SampledJointGoal: trialData.alwaysCommittedAgentPlayer2SampledJointGoal,
    alwaysCommittedAgentPlayer1SampledJointGoalWeights: trialData.alwaysCommittedAgentPlayer1SampledJointGoalWeights,
    alwaysCommittedAgentPlayer2SampledJointGoalWeights: trialData.alwaysCommittedAgentPlayer2SampledJointGoalWeights,
    alwaysCommittedAgentPlayer1SampledJointGoalHistory: trialData.alwaysCommittedAgentPlayer1SampledJointGoalHistory,
    alwaysCommittedAgentPlayer2SampledJointGoalHistory: trialData.alwaysCommittedAgentPlayer2SampledJointGoalHistory,
...
function computeSummary(trials, meta) {
  ...
  for (const trial of trials) {
    if (!sessions.has(trial.sessionIndex)) sessions.set(trial.sessionIndex, []);
...
function runSimulation({ sessions, trialsPerSession, seed, sessionOffset, lambda, beta }) {
  ...
  const originalLambda = CONFIG.game.agent.alwaysCommitted.lambda;
  const originalBeta = CONFIG.game.agent.alwaysCommitted.beta;

  CONFIG.game.players.player1.type = 'alwaysCommittedAgent';
  CONFIG.game.players.player2.type = 'alwaysCommittedAgent';
  CONFIG.game.agent.type = 'joint';
  CONFIG.game.experiments.order = ['2P3G'];
  CONFIG.game.experiments.numTrials['2P3G'] = trialsPerSession;
  if (Number.isFinite(lambda)) CONFIG.game.agent.alwaysCommitted.lambda = lambda;
  if (Number.isFinite(beta)) CONFIG.game.agent.alwaysCommitted.beta = beta;
  ...
  try {
    ...
    return withSuppressedConsole(() => withSynchronousTimers(() => {
      ...
      for (let localSessionIndex = 0; localSessionIndex < sessions; localSessionIndex++) {
        ...
        withSeededRandom(sessionSeed, () => {
          ...
          const agent1 = new AlwaysCommittedAgent({
            rlAgent: sharedRl,
            lambda: CONFIG.game.agent.alwaysCommitted.lambda,
            beta: CONFIG.game.agent.alwaysCommitted.beta
          });
          const agent2 = new AlwaysCommittedAgent({
            rlAgent: sharedRl,
            lambda: CONFIG.game.agent.alwaysCommitted.lambda,
            beta: CONFIG.game.agent.alwaysCommitted.beta
          });
    ...
    CONFIG.game.agent.alwaysCommitted.lambda = originalLambda;
    CONFIG.game.agent.alwaysCommitted.beta = originalBeta;
  }
}

function main() {
  const { sessions, trialsPerSession, seed, sessionOffset, lambda, beta, outputDir: outputDirArg } = parseArgs(process.argv.slice(2));
  const result = runSimulation({ sessions, trialsPerSession, seed, sessionOffset, lambda, beta });

  const outputDir = path.resolve(outputDirArg);
  fs.mkdirSync(outputDir, { recursive: true });

  const lambdaPart = Number.isFinite(lambda) ? `lambda_${formatNumberForPath(lambda)}_` : '';
  const betaPart = Number.isFinite(beta) ? `beta_${formatNumberForPath(beta)}_` : '';
  const suffix = `${betaPart}${lambdaPart}sessions_${sessionOffset}_to_${sessionOffset + sessions - 1}`;
  const summaryPath = path.join(outputDir, `always_committed_vs_always_committed_2p3g_summary_${suffix}.json`);
  const trialsPath = path.join(outputDir, `always_committed_vs_always_committed_2p3g_trials_${suffix}.json`);
  const rawTrialsPath = path.join(outputDir, `always_committed_vs_always_committed_2p3g_raw_trials_${suffix}.json`);
