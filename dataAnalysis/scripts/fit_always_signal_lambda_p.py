#!/usr/bin/env python3
"""Fit AlwaysSignalAgent lambda x mixture-p against trial/player-level human data."""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "dataAnalysis" / "model_model" / "signal_agent" / "outputs" / "signal_agent_from_start_lambda_p_fit"
DEFAULT_RAW_DIR = PROJECT_ROOT / "dataAnalysis" / "raw_data" / "model_model_simulations" / "signal_agent" / "from_start_lambda_p_fit"
...
def measure_row(lambda_value: float, p_signal: float, raw_trials: List[Dict[str, Any]], sim_df: pd.DataFrame) -> Dict[str, float]:
    ...
    for prefix, condition in [("average", None), ("equal", "equal_to_both")]:
        rows = comparison_rows("sampleJointGoalAndSignal_fromStart", raw_trials, sim_df, condition)
...
def write_notebook(output_dir: Path, best_row: Dict[str, Any]) -> Path:
    nb_dir = PROJECT_ROOT / "dataAnalysis" / "model_model" / "signal_agent" / "notebooks" / "signal_agent_from_start_lambda_p_fit"
    nb_dir.mkdir(parents=True, exist_ok=True)
    nb_path = nb_dir / "AlwaysSignalAgent_from_start_lambda_p_results.ipynb"
    cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": [
                "# sampleJointGoalAndSignal_fromStart Lambda x p Fit\n",
                "\n",
                "Always-on posterior timing with SignalAgent mixture action policy.\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "grid = pd.read_csv(OUT / 'always_signal_lambda_p_grid.csv')\n",
                "best_metrics = pd.read_csv(OUT / 'always_signal_lambda_p_best_metrics.csv')\n",
                "all_summary = pd.read_csv(OUT / 'all_distance_always_signal_human_4panel_summary.csv')\n",
                "equal_summary = pd.read_csv(OUT / 'equal_to_both_always_signal_human_4panel_summary.csv')\n",
            ],
        },
...
def evaluate_setting(
    ...
    sim_df = add_measures(long_player_rows(raw_trials, "sampleJointGoalAndSignal_fromStart"))
...
def main() -> None:
    ...
    best_df = add_measures(long_player_rows(best_raw, "sampleJointGoalAndSignal_fromStart"))
    ...
    grid_path = args.output_dir / "always_signal_lambda_p_grid.csv"
    best_metrics_path = args.output_dir / "always_signal_lambda_p_best_metrics.csv"
    equal_csv = args.output_dir / "equal_to_both_always_signal_human_4panel_summary.csv"
    all_csv = args.output_dir / "all_distance_always_signal_human_4panel_summary.csv"
    grid_df.to_csv(grid_path, index=False)
    pd.DataFrame([best_row]).to_csv(best_metrics_path, index=False)
    equal_df.to_csv(equal_csv, index=False)
    all_df.to_csv(all_csv, index=False)

    average_heatmap = args.output_dir / "always_signal_lambda_p_average_4measure_heatmaps.png"
    equal_heatmap = args.output_dir / "always_signal_lambda_p_equal_to_both_4measure_heatmaps.png"
    plot_lambda_p_heatmaps(
        grid_df,
        best_row,
        average_heatmap,
        "average",
        "sampleJointGoalAndSignal_fromStart: lambda x p sweep, average all 2P3G",
    )
    plot_lambda_p_heatmaps(
        grid_df,
        best_row,
        equal_heatmap,
        "equal",
        "sampleJointGoalAndSignal_fromStart: lambda x p sweep, equal-to-both",
    )
    plot_comparison(
        equal_df,
        args.output_dir / "equal_to_both_always_signal_human_4panel.png",
        "Equal-to-Both Only: sampleJointGoalAndSignal_fromStart and Human-Human",
    )
    plot_comparison(
        all_df,
        args.output_dir / "all_distance_always_signal_human_4panel.png",
        "All Distance Conditions: sampleJointGoalAndSignal_fromStart and Human-Human",
    )
    notebook_path = None if args.skip_notebook else write_notebook(args.output_dir, best_row)

    summary = {
        "model": "sampleJointGoalAndSignal_fromStart",
    ...
    summary_path = args.output_dir / "always_signal_lambda_p_fit_summary.json"
