...
equivalent to $s_{\text{logp}}(a) = \log P(g^{*} \mid a) = -\mathrm{KL}\bigl(\delta_{g^{*}} \,\|\, P(\cdot \mid a)\bigr)$. Standard pragmatic-speaker form (Frank & Goodman 2012). Limits: $\alpha=0$ -> base policy; $\alpha=1$ -> base x posterior; $\alpha\to\infty$ -> argmax of $P(g^{*} \mid a)$.
...
- $\pi_{\text{committed}}(a \mid g^{*}) = \delta_{a^\circ}$ where $a^\circ = \operatorname{getJointRLAction}(s, g^{*})$ — the deterministic CommittedAgent action toward target $g^{*}$.
...
- **Commitment** $\in \{0, 1\}$: $\mathbf{1}[\text{firstDetectedSharedGoal} = \text{finalReachedGoal}]$.
...
$$\mathcal{L}(\theta) \;=\; \sum_{c} \left[\, -k_c \log \hat p_c(\theta) \,-\, (n_c - k_c) \log\bigl(1 - \hat p_c(\theta)\bigr) \,\right]$$
...
Raw `signal_vs_signal_2p3g_raw_trials_*.json` files live under `dataAnalysis/raw_data/model_model_simulations/signal_agent/`.
